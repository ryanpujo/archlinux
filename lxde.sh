...
sudo pacman -Syu xorg xorg-xinit lxappearance lxinput lxrandr lxde lxdm lxterminal pcmanfm xarchiver leafpad p7zip unrar xz libreoffice gst-libav gst-plugin-pipewire gst-plugins-bad gst-plugins-ugly gstreamer-vaapi archlinux-wallpaper firefox vlc papirus-icon-theme mtpfs gvfs gvfs-mtp dbeaver docker docker-compose jdk21-openjdk ttf-fira-code go rhythmbox terminus-font powerline-fonts ttf-font-awesome adobe-source-code-pro-fonts ttf-jetbrains-mono noto-fonts noto-fonts-emoji noto-fonts-extra awesome-terminal-fonts

yay -S jmtpfs visual-studio-code-bin insomnia-bin ttf-ms-fonts ttf-meslo-nerd-font-powerlevel10k
