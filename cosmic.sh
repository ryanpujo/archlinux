...
sudo pacman -Syu xorg xorg-xinit cosmic ark p7zip unrar xz libreoffice-fresh gst-libav gst-plugin-pipewire gst-plugins-bad gst-plugins-ugly gstreamer-vaapi archlinux-wallpaper firefox vlc papirus-icon-theme mtpfs gvfs-mtp dbeaver docker docker-compose jdk21-openjdk ttf-fira-code go rhythmbox terminus-font powerline-fonts ttf-font-awesome adobe-source-code-pro-fonts ttf-jetbrains-mono noto-fonts noto-fonts-emoji noto-fonts-extra awesome-terminal-fonts htop
yay -S jmtpfs visual-studio-code-bin intellij-idea-ultimate-edition insomnia-bin ttf-ms-fonts ttf-meslo-nerd-font-powerlevel10k
